...
public class LinkedIntList{
    ...
    public void Add(int value){
        ...
        // using a while loop to go to the position before the null value
            ...
            // now we create the new node, assign the value given to it and link the last position in the LL to the new node.
    ...
    public void Remove() {
        // I am removing the last node
         ...
         while(node.Next != null){
            // now in the while loop we first set the new variable to the position f node and
        // then increase node by 1. The order is important, otherwise both would have the same position
        PreviousNode = node;
        node = node.Next;
     }
...
// I start like in the remove function to remove the first value found
    IntNode node = Head;
    IntNode PreviousNode = null;
...
// if it's not empty I use a counting function and return the size int variable to the demand. In the main I implemented a Console.WriteLine that prints the outcome
    else{
        int size = 0;
        while(node != null){
        size ++;
...
// here I create a while loop where I write every value, until we reach the end of the LL
    IntNode node = Head;
    while (node != null)
    {   
